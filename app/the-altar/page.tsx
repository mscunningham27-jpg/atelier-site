...
export default function TheAltarPage() {
  const articles = getArticlesBySection('the-altar')
  return (
      ...
      <header style={{ borderBottom: '0.5px solid var(--border)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 24px 0' }}>
          ...
          <nav style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(14px, 3vw, 40px)', padding: '16px 0' }}>
            {SECTIONS.map((s) => (
              <Link key={s.name} href={s.path} className="nav-link" style={s.path === '/the-altar' ? { color: 'var(--accent-altar)', borderBottomColor: 'var(--accent-altar)' } : {}}>{s.name}</Link>
            ))}
          </nav>
        </div>
      </header>
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px 80px' }}>
        <div style={{ background: 'var(--room-altar)', borderBottom: '0.5px solid var(--border)', padding: 'clamp(40px, 5vw, 72px) 24px clamp(32px, 4vw, 56px)', textAlign: 'center', margin: '0 -24px 48px' }}>
          <span style={{ fontFamily: 'var(--font-altar)', fontSize: '14px', color: 'var(--accent-altar)', opacity: 0.8, display: 'block', marginBottom: '8px' }}>†</span>
          <h1 style={{ fontFamily: 'var(--font-altar)', fontSize: 'clamp(36px, 5vw, 68px)', fontWeight: 400, letterSpacing: '0.04em', color: 'var(--ink)', margin: '0 0 16px', lineHeight: 1.05 }}>The Altar</h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(13px, 1.4vw, 15px)', fontWeight: 300, color: 'var(--ink-mid)', lineHeight: 1.7, margin: '0 auto', maxWidth: '520px' }}>Prayer, Scripture, lament, and the inner life. For the honest questions — the ones you bring to God and the ones you are afraid to.</p>
        </div>
        {articles.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <p style={{ fontFamily: 'var(--font-altar)', fontSize: '22px', letterSpacing: '0.04em', color: 'var(--ink-muted)' }}>The first pieces are on their way.</p>
          </div>
        ) : (
            ...
            {articles.map((article, i) => (
              <Link key={article.slug} href={`/articles/${article.slug}`} className="article-link" style={{ borderBottom: i < articles.length - 1 ? '0.5px solid var(--border)' : 'none', display: 'grid', gridTemplateColumns: article.heroImage ? '200px 1fr' : '1fr', minHeight: '160px' }}>
                ...
                <div style={{ padding: 'clamp(20px, 3vw, 36px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h2 className="article-headline" style={{ fontFamily: 'var(--font-altar)', fontSize: 'clamp(20px, 2.4vw, 30px)', fontWeight: 400, letterSpacing: '0.04em', color: 'var(--ink)', lineHeight: 1.2, margin: '0 0 12px' }}>{article.title}</h2>
