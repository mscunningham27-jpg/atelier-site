...
export default function TheGuildPage() {
  const articles = getArticlesBySection('the-guild')
  return (
      ...
      <header style={{ borderBottom: '0.5px solid var(--border)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 24px 0' }}>
          ...
          <nav style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(14px, 3vw, 40px)', padding: '16px 0' }}>
            {SECTIONS.map((s) => (
              <Link key={s.name} href={s.path} className="nav-link" style={s.path === '/the-guild' ? { color: 'var(--accent-guild)', borderBottomColor: 'var(--accent-guild)' } : {}}>{s.name}</Link>
            ))}
          </nav>
        </div>
      </header>
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px 80px' }}>
        <div style={{ background: 'var(--room-guild)', borderBottom: '0.5px solid var(--border)', padding: 'clamp(40px, 5vw, 72px) 24px clamp(32px, 4vw, 56px)', textAlign: 'center', margin: '0 -24px 48px' }}>
          <span style={{ fontFamily: 'var(--font-guild)', fontSize: '14px', color: 'var(--accent-guild)', opacity: 0.8, display: 'block', marginBottom: '8px' }}>◎</span>
          <h1 style={{ fontFamily: 'var(--font-guild)', fontSize: 'clamp(36px, 5vw, 68px)', fontWeight: 400, color: 'var(--ink)', margin: '0 0 16px', lineHeight: 1.05 }}>The Guild</h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(13px, 1.4vw, 15px)', fontWeight: 300, color: 'var(--ink-mid)', lineHeight: 1.7, margin: '0 auto', maxWidth: '520px' }}>Community, spotlights, and the wider movement. Where makers are seen, the HearthLight family gathers, and the creative and spiritual life meet.</p>
        </div>
        {articles.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <p style={{ fontFamily: 'var(--font-guild)', fontSize: '22px', fontStyle: 'italic', color: 'var(--ink-muted)' }}>The first pieces are on their way.</p>
          </div>
        ) : (
            ...
            {articles.map((article, i) => (
              <Link key={article.slug} href={`/articles/${article.slug}`} className="article-link" style={{ borderBottom: i < articles.length - 1 ? '0.5px solid var(--border)' : 'none', display: 'grid', gridTemplateColumns: article.heroImage ? '200px 1fr' : '1fr', minHeight: '160px' }}>
                ...
                <div style={{ padding: 'clamp(20px, 3vw, 36px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h2 className="article-headline" style={{ fontFamily: 'var(--font-guild)', fontSize: 'clamp(20px, 2.4vw, 30px)', fontWeight: 400, color: 'var(--ink)', lineHeight: 1.2, margin: '0 0 12px' }}>{article.title}</h2>
