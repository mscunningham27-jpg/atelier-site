...
export default function AssignmentPage() {
  const articles = getArticlesBySection('the-assignment')
  ...
  return (
    <div style={{ backgroundColor: 'var(--parchment)', minHeight: '100vh' }}>
      ...
      <header style={{ borderBottom: '0.5px solid var(--border)' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '28px 24px 0' }}>
          ...
          <nav style={{ display: 'flex', justifyContent: 'center', gap: 'clamp(14px, 3vw, 40px)', padding: '16px 0' }}>
            {SECTIONS.map((s) => (
              <Link key={s.name} href={s.path} className="nav-link" style={s.path === '/the-assignment' ? { color: 'var(--accent-assignment)', borderBottomColor: 'var(--accent-assignment)' } : {}}>{s.name}</Link>
            ))}
          </nav>
        </div>
      </header>
      ...
      <main style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 24px 80px' }}>
        ...
        {/* ── Section header ───────────────────────────────────── */}
        <div style={{ background: 'var(--room-assignment)', borderBottom: '0.5px solid var(--border)', padding: 'clamp(40px, 5vw, 72px) 0 clamp(32px, 4vw, 56px)', textAlign: 'center', margin: '0 -24px 48px', paddingLeft: '24px', paddingRight: '24px' }}>
          <span style={{ fontFamily: 'var(--font-assignment)', fontSize: 'clamp(10px, 1.2vw, 14px)', letterSpacing: '0.06em', color: 'var(--accent-assignment)', opacity: 0.8, display: 'block', marginBottom: '8px' }}>✦</span>
          <h1 style={{ fontFamily: 'var(--font-assignment)', fontSize: 'clamp(36px, 5vw, 68px)', fontWeight: 400, color: 'var(--ink)', margin: '0 0 16px', lineHeight: 1.05 }}>The Assignment</h1>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: 'clamp(13px, 1.4vw, 15px)', fontWeight: 300, color: 'var(--ink-mid)', lineHeight: 1.7, margin: '0 auto', maxWidth: '520px' }}>
            Theology of creativity and calling. The why beneath the work — for artists, makers, and anyone who has wondered if their creative life is a sacred one.
          </p>
        </div>
        ...
        {articles.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            <p style={{ fontFamily: 'var(--font-assignment)', fontSize: '22px', fontStyle: 'italic', color: 'var(--ink-muted)' }}>The first pieces are on their way.</p>
          </div>
        ) : (
          <div style={{ display: 'grid', gap: '0', border: '0.5px solid var(--border)', borderRadius: '2px', overflow: 'hidden' }}>
            {articles.map((article, i) => (
              <Link key={article.slug} href={`/articles/${article.slug}`} className="article-link" style={{ borderBottom: i < articles.length - 1 ? '0.5px solid var(--border)' : 'none', display: 'grid', gridTemplateColumns: article.heroImage ? '200px 1fr' : '1fr', minHeight: '160px' }}>
                {article.heroImage && (
                  <div style={{ overflow: 'hidden', background: 'var(--parchment-mid)' }}>
                ...
                <div style={{ padding: 'clamp(20px, 3vw, 36px)', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  <h2 className="article-headline" style={{ fontFamily: 'var(--font-assignment)', fontSize: 'clamp(20px, 2.4vw, 30px)', fontWeight: 400, color: 'var(--ink)', lineHeight: 1.2, margin: '0 0 12px' }}>{article.title}</h2>
